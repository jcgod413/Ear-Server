...
module.exports = {
  certificate({ mdn }) {
    return User
      ...
      .then((user) => {
        if (user) { throw new ConflictError('이미 등록된 회원입니다.'); }

        const certificateNum = Math.floor(Math.random() * 9000) + 1000;
        const contents = '안녕하세요 설리번입니다.\n인증번호는 ' + certificateNum + ' 입니다.';
        console.log(contents);

        return redis.publish(config.REDIS_HEADER_USER + mdn, certificateNum)
          .catch((e) => {
            throw e;
          })
          .then(() => {
  ...
  verify({ mdn, certificateNum }) {
    ...
    return redis.subscribe(config.REDIS_HEADER_USER + mdn)
      .catch((e) => {
        throw e;
      })
      .then((savedNum) => {
  ...
  login({ mdn, password }) {
    ...
    return User
      ...
      .then((user) => {
        if (!user) { throw new NotExistError('일치하는 회원 정보가 없습니다.'); }
  ...
  signup({ name, mdn, password, gender, districtId }) {
    ...
    return User
      ...
      .then((user) => {
        if (user) { throw new ConflictError('이미 가입된 회원입니다.'); }
